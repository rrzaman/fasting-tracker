...
- **Infrastructure:** Terraform (modules pattern, state in versioned S3 backend), API Gateway HTTP API
- **Auth:** AWS Cognito
...
`terraform/environments/prod/terraform.tfvars` (`alarm_email`), `~/.aws/credentials`.

**Terraform state is in S3**, not a local file: bucket `fasting-tracker-tfstate-rayyan`,
key `prod/terraform.tfstate`, versioned, native locking (ADR-026). The bucket was
created with the AWS CLI and is deliberately not managed by Terraform. Requires
Terraform >= 1.16. On a machine holding an old local `terraform.tfstate`, use
`terraform init -reconfigure`, never `-migrate-state`, which would offer to
